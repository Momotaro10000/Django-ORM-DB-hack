...
def get_kid_account(kid_name):
    try:
        ...
        print('Учеников с именем {} слишком много, возможно стоит указать полное имя.'.format(kid_name))
        return None
...
def create_commendation(kid_name,subject):
    commendations = [ 'Молодец!', 'Отлично!', 'Хорошо!', 'Гораздо лучше, чем ожидалось!',  
                      'Великолепно!', 'Прекрасно!', 'Сказано здорово – просто и ясно!', 
    ...
    try:
        lesson = Lesson.objects.filter(year_of_study=kid.year_of_study,group_letter=kid.group_letter,subject__title=subject).order_by('date').first()
        Commendation.objects.create(text=random.choice(commendations),created=lesson.date,schoolkid=kid,subject=lesson.subject,teacher=lesson.teacher)
    except ObjectDoesNotExist:
